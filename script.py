...
def detect_labels(image, max_labels=10, min_confidence=60):
...
def detect_ppe(image):
    ...
    ppe_response = rekognition.detect_protective_equipment(Image={'Bytes': imgbytes},
    SummarizationAttributes={'MinConfidence':80, 'RequiredEquipmentTypes':['FACE_COVER','HEAD_COVER']})
...
def analyze():
    ...
    if flag:
        ...
        print("end of objects detected")
                ...
                #ppe detection
        ppe_detected = detect_ppe(snapshot_url)
        ppe = 0
        hard_hat_ppe = 0 
        mask_ppe = 0
        for person in ppe_detected['Persons']:            
                    ...
                    if len(ppe_items) ==0:
                        ...
                        for ppe_item in ppe_items:
                            if ppe_item['Type'] == 'HEAD_COVER':
                                ...
                                hard_hat_ppe = (int(round(ppe_item['Confidence'],3)))
                                ppe_type = ppe_item['Type']
                                print(hard_hat_ppe)
                        else:
                            if ppe_item['Type'] == 'FACE_COVER':
                                print('\t\t' + ppe_item['Type'] + '\n\t\t\tConfidence: ' + str(ppe_item['Confidence'])) 
                                print('\t\tCovers body part: ' + str(ppe_item['CoversBodyPart']['Value']) + '\n\t\t\tConfidence: ' + str(ppe_item['CoversBodyPart']['Confidence']))
                                mask_ppe = (int(round(ppe_item['Confidence'],3)))
                                ppe_type = ppe_item['Type']
                                print(mask_ppe)
                                
                print()
            print()
            #Publish ppe via MQTT to NodeRed
            client.publish("hard_hat_ppe", hard_hat_ppe)
            client.publish("mask_ppe",mask_ppe)
            client.publish("ppe_type",ppe_type)
